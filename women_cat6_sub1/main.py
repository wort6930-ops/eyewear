...
# Hardcoded subcategory URLs for Eyewear
SUBCATEGORY_URLS = [
    "https://www.boutiqaat.com/ar-kw/women/eyewear/sunglasses/l/",
    "https://www.boutiqaat.com/ar-kw/women/accessories/sunglasses-accessories/l/",
    "https://www.boutiqaat.com/ar-kw/women/eyewear/optical-frames/l/",
]
...
class BoutiqaatDataPipeline:
    ...
    def _upload_product_image(self, product: Dict, category_name: str, subcategory_name: str) -> str:
        ...
        try:
            ...
            s3_path = (
                f"boutiqaat-data/year={datetime.now().strftime('%Y')}/month={datetime.now().strftime('%m')}/day={datetime.now().strftime('%d')}/eyewear/images/"
                f"{safe_category}"
            )
    ...
    def _upload_excel_file(self, local_path: str, category_name: str) -> bool:
        ...
        try:
            ...
            s3_path = (
                f"boutiqaat-data/year={datetime.now().strftime('%Y')}/month={datetime.now().strftime('%m')}/day={datetime.now().strftime('%d')}/eyewear/excel-files"
            )
